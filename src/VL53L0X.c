...
// General Buffers
//-----------------------------------------------------------------------------
static uint8_t xdata tx_buf[4];
static uint8_t xdata rx_buf[4];

//-----------------------------------------------------------------------------
// VL53L0X Core Functions  (Replace with your implentation)
//-----------------------------------------------------------------------------
...
{
  tx_buf[0] = value;
  I2C_WriteDevice(dev->addr, reg, tx_buf, 1);
}
...
void VL53L0X_writeReg16Bit(struct VL53L0X *dev, uint8_t reg, uint16_t value)
{
  ...
  I2C_WriteDevice(dev->addr, reg, tx_buf, 2);
}

// Write a 32-bit register
void VL53L0X_writeReg32Bit(struct VL53L0X *dev, uint8_t reg, uint32_t value)
{
  tx_buf[0] = (uint8_t)(value >> 24);
  tx_buf[1] = (uint8_t)(value >> 16);
  tx_buf[2] = (uint8_t)(value >> 8);
  tx_buf[3] = (uint8_t)(value & 0xFF);
  I2C_WriteDevice(dev->addr, reg, tx_buf, 4);
}
...
{
  xdata uint8_t value;
  I2C_ReadDevice(dev->addr, reg, &value, 1);
  return value;
}
...
{
  xdata uint16_t value;
  I2C_ReadDevice(dev->addr, reg, rx_buf, 2);
  value = (uint16_t)(rx_buf[0] << 8);
  value |= (uint16_t)rx_buf[1];
  return value;
}
...
{
  xdata uint32_t value;
  I2C_ReadDevice(dev->addr, reg, rx_buf, 4);
  value = (uint32_t)(rx_buf[0] << 24);
  value |= (uint32_t)(rx_buf[1] << 16);
  value |= (uint32_t)(rx_buf[2] << 8);
...
{
  I2C_WriteDevice(dev->addr, reg, src, count);
}
...
{
  I2C_ReadDevice(dev->addr, reg, dst, count);
}

//-----------------------------------------------------------------------------
// Timeout Function (Replace with your implentation)
//-----------------------------------------------------------------------------
void VL53L0X_startTimeout(struct VL53L0X *dev)
{
  Countdown_ms(dev->io_timeout);
}

bool VL53L0X_checkTimeoutExpired(struct VL53L0X *dev)
{
  return Timeout_CheckExpired();
}

//-----------------------------------------------------------------------------
// VL53L0X Control Functions
//-----------------------------------------------------------------------------
...
uint16_t VL53L0X_readRangeContinuousMillimeters(struct VL53L0X *dev)
{
  ...
  range = VL53L0X_readReg16Bit(dev, RESULT_RANGE_STATUS + 10);
...
//-----------------------------------------------------------------------------
// VL53L0X Init/Calibration Functions
//-----------------------------------------------------------------------------
...
{
  // modification for ancient C
  uint8_t spad_count;
  unsigned char spad_type_is_aperture;
  xdata uint8_t ref_spad_map[6];
...
//  measurements. Increasing the budget by a factor of N decreases the range
// measurement standard deviation by a factor of sqrt(N).
...
bool VL53L0X_setMeasurementTimingBudget(struct VL53L0X *dev, uint32_t budget_us)
{
  ...
  xdata struct VL53L0X_SequenceStepEnables enables;
  xdata struct VL53L0X_SequenceStepTimeouts timeouts;
...
bool VL53L0X_setVcselPulsePeriod(struct VL53L0X *dev, enum VL53L0X_vcselPeriodType type, uint8_t period_pclks)
{
  ...
  sequence_config = VL53L0X_readReg(dev, SYSTEM_SEQUENCE_CONFIG);
  VL53L0X_writeReg(dev, SYSTEM_SEQUENCE_CONFIG, 0x02);
...
bool VL53L0X_getSpadInfo(struct VL53L0X *dev, uint8_t *count, unsigned char *type_is_aperture)
{
  uint8_t tmp;
...
// Based on VL53L0X_set_offset_calibration_data_micro_meter(), range:{-512, 511}
void VL53L0X_setOffsetMilliMeter(struct VL53L0X *dev, int16_t OffsetCalibrationDataMilliMeter)
{
  xdata int16_t cMaxOffsetMilliMeter = 511;
  xdata int16_t cMinOffsetMilliMeter = -512;
  xdata int16_t cOffsetRange = 4096;
  uint16_t encodedOffsetVal;

  if (OffsetCalibrationDataMilliMeter > cMaxOffsetMilliMeter)
  {
    OffsetCalibrationDataMilliMeter = cMaxOffsetMilliMeter;
  }
  else if (OffsetCalibrationDataMilliMeter < cMinOffsetMilliMeter)
  {
    OffsetCalibrationDataMilliMeter = cMinOffsetMilliMeter;
  }

  /* The offset register is 10.2 format and units are mm
   * therefore conversion is appliedby a  multiplication of 4 (1000/250).
   */
  if (OffsetCalibrationDataMilliMeter >= 0)
  {
    encodedOffsetVal =
        OffsetCalibrationDataMilliMeter * 4;
  }
  else
  {
    encodedOffsetVal = cOffsetRange + OffsetCalibrationDataMilliMeter * 4;
  }

  VL53L0X_writeReg16Bit(dev,
